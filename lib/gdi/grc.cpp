...
{
    switch (o->opcode) {
        case gOpcode::setScale:
        case gOpcode::setOpacity:
        case gOpcode::setTransform:
        case gOpcode::startAnimation:
        case gOpcode::stopAnimation:
        case gOpcode::updateAnimation:
#if defined(USE_LIBVUGLES2) || defined(HAVE_MALI)
            if (o->opcode == gOpcode::updateAnimation && o->parm.animation) {
                updateTransformation(
                    o->parm.animation->scale,
                    o->parm.animation->opacity,
                    o->parm.animation->translation
                );
            }
#endif
            break;
...
void gDC::initOpenGLES()
{
    ...
    // Context attributes (OpenGL ES 2.0)
    ...
    if (m_eglDisplay == EGL_NO_DISPLAY) {
        eDebug("[gDC] Error: Failed to get EGL display");
        return;
    }
    ...
    if (!eglInitialize(m_eglDisplay, &majorVersion, &minorVersion)) {
        eDebug("[gDC] Error: Failed to initialize EGL");
        return;
    }
    ...
    if (!eglChooseConfig(m_eglDisplay, configAttribs, &config, 1, &numConfigs)) {
        eDebug("[gDC] Error: Failed to choose EGL config");
        return;
    }

    // Create window surface
    EGLNativeWindowType nativeWindow = (EGLNativeWindowType)m_pixmap->surface;
    m_eglSurface = eglCreateWindowSurface(m_eglDisplay, config, nativeWindow, NULL);
    if (m_eglSurface == EGL_NO_SURFACE) {
        eDebug("[gDC] Error: Failed to create window surface");
        return;
    }
    ...
    if (m_eglContext == EGL_NO_CONTEXT) {
        eDebug("[gDC] Error: Failed to create EGL context");
        return;
    }

    // Make current
    if (!eglMakeCurrent(m_eglDisplay, m_eglSurface, m_eglSurface, m_eglContext)) {
        eDebug("[gDC] Error: Failed to make context current");
        return;
    }
    ...
    eDebug("[gDC] OpenGL ES 2.0 initialized successfully");
}

void gDC::cleanupOpenGLES()
{
    if (m_eglDisplay != EGL_NO_DISPLAY) {
        ...
        // Release EGL context and surface
    ...
    eDebug("[gDC] OpenGL ES resources cleaned up");
}

void gDC::updateTransformation(float scale, float opacity, ePoint translation)
{
    if (!m_glesInitialized || m_eglDisplay == EGL_NO_DISPLAY || !m_shaderProgram)
        return;
    ...
    // Update uniforms
    GLint mvpLocation = glGetUniformLocation(m_shaderProgram, "u_mvpMatrix");
    GLint opacityLocation = glGetUniformLocation(m_shaderProgram, "u_opacity");
    GLint colorLocation = glGetUniformLocation(m_shaderProgram, "u_color");
